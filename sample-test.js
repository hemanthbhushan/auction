...
describe(" transfer rewards",()=>{
  it("checking transfer rewards",async ()=>{
   ...
   const checkTransferRewards= await auction.connect(owner).transferRewards(token.address);
  //  expect(checkTransferRewards).to.equal(checkWinner);

    expect(await token.balanceOf(owner.address)).to.equal(userBalance+1022);
